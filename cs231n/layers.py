...
def affine_backward(dout, cache):
    ...
    # no averaging in the answer, why?? Is the loss defined as the sum instead 
    # of the mean across all the examples?
    # dw = x_flat.T@dout / N
    dw = x_flat.T@dout
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        # astype was necessary if I wrote two lines.
        # mask = np.random.rand(*(x.shape))<p
        # mask = mask.astype(float)/p
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out_trans, cache = batchnorm_forward(x_trans, gamma, beta, bn_param)
